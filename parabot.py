...
def parseArgs(argv):
    ...
    """
    global includeTags, excludeTags, suite_name, clientCwd, forceSerialExecution, baseDir, logFolder, testsToRun, suiteToRun, testDirectory, listenerVal
    if len(argv)<1:
        usage()
        sys.exit(2)
...
class Jybot():
    ...
    def start(self, tests, suite, args=[]):
        ...
        jybotCommand = jybotCommand + "--listener %s " % os.path.join(listenerVal)
        #jybotCommand = jybotCommand + ' --listener C:\Python27\lib\site-packages\robotide\contrib\testrunner\TestRunnerAgent.py:57587 '
...
for test in listOfTests:
    ...
    for testToRun in testsToRun:
        if test.longname == testToRun:
            ...
            if series==-1:
                para_tests.append(test.longname)
            if series==1:
                seri_start_tests.append(test.longname)
            if series==2:
                seri_end_tests.append(test.longname)
